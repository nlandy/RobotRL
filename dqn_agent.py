...
class Agent():
    ...
    def learn(self, experiences, gamma):
        ...
        Q_targets_next = self.qnetwork_target.probs(next_states.float()).detach().max(1)[0].unsqueeze(1)
        # Compute Q targets for current states
        Q_targets = rewards.float() + (gamma * Q_targets_next * (1 - dones.float()))

        # Get expected Q values from local model
        Q_expected = self.qnetwork_local.probs(states.float())
